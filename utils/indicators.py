...
def williams_r(
    ...
    for i in range(period - 1, len(closes)):
        ...
        result[i] = ((hi - closes[i]) / (hi - lo) * -100) if hi != lo else -50.0
    return result
...
def detect_regime(closes: np.ndarray, period: int = 50) -> str:
    ...
    ema_s = ema(c, 10)
    ema_l = ema(c, 30)
    diff  = abs(ema_s[-1] - ema_l[-1]) / mean if mean != 0 else 0
...
def compute_all(candles: List[Dict]) -> Dict:
    ...
    return {
        ...
        "bb_upper":     round(float(_bb["upper"][last]), 5),
        "bb_mid":       round(float(_bb["mid"][last]),   5),
        "bb_lower":     round(float(_bb["lower"][last]), 5),
        "bb_width":     round(float(_bb["upper"][last] - _bb["lower"][last]), 5),
        ...
        "price_vs_bb":  (
            "above" if closes[last] > _bb["upper"][last] else
            "below" if closes[last] < _bb["lower"][last] else "inside"
        ),
    }
